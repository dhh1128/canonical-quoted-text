...
## Implementations
A reference implementation exists in [python](cqt.py), with unit tests in [test_cqt.py](test_cqt.py). See also ports in [javascript](cqt.js), [java](Cqt.java), [go](cqt.go), [rust](cqt.rs), and [swift](Cqt.swift). Code is in the public domain; see [LICENSE](https://github.com/dhh1128/canonical-quoted-text/blob/main/LICENSE).
